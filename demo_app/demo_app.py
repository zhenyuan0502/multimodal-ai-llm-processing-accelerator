...
with gr.Blocks(analytics_enabled=False) as sum_text_block:
    ...
    # Output components
    with gr.Column(render=False) as restore_output_row:
        restore_output_label = gr.Label(value="API Response", show_label=False)
        with gr.Row():
            restore_status_code = gr.Textbox(
                label="Response Status Code", interactive=False
            )
            sum_text_time_taken = gr.Textbox(label="Time Taken", interactive=False)
        restore_output_text = gr.Textbox(label="API Response", interactive=False)
    restore_get_response_btn = gr.Button("Process Text", variant="primary")
    restore_output_row.render()
    # Actions
    restore_get_response_btn.click(
...
### Restore analyzer schemas ###
with gr.Blocks(analytics_enabled=False) as restore_block:
    def restore_send_request():
        return send_request(
            route="create_content_understanding_schema",
            json_data={},
            force_json_content_type=False,
        )

    # Input components
    restore_instructions = gr.Markdown(
        (
            "Restores the analyzer schemas used in the demo app. "
        ),
        show_label=False,
    )
    ...
    with gr.Column(render=False) as restore_output_row:
        ...
        restore_output_text = gr.Textbox(label="API Response", interactive=False)
    restore_get_response_btn = gr.Button("Restore analyzer schemas", variant="primary")
    restore_output_row.render()
    # Actions
    restore_get_response_btn.click(
        fn=restore_send_request,
        inputs=[],
        outputs=[
            restore_status_code,
            restore_output_text,
...
with gr.Blocks(
    ...
    with gr.Tab("Restore analyzer schemas"):
        
        restore_block.render()
